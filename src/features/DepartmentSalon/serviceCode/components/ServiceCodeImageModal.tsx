...
import { useUploadServiceCodeImageMutation } from '../services/serviceCodeApi'
import { getApiError } from '@/services/apiHelpers'

const MAX_SIZE = 5 * 1024 * 1024

interface ServiceCodeImageModalProps {
  open: boolean
  onClose: () => void
...
export default function ServiceCodeImageModal({
  open,
  onClose,
  serviceCodeId,
  serviceCodeName,
}: ServiceCodeImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadServiceCodeImageMutation()
  ...
  const handleUpload = async () => {
    if (!preview) return
    try {
      await uploadImage({ serviceCodeId, file: preview.file }).unwrap()
  ...
  return (
    <Modal
      open={open}
      onClose={handleClose}
      title="Upload ServiceCode Image"
      size="sm"
      footer={
        <>
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          Uploading image for{' '}
          <span className="font-medium text-[var(--text-primary)]">{serviceCodeName}</span>
        </p>
