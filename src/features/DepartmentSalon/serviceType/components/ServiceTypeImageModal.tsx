...
import { useUploadServiceTypeImageMutation } from '../services/serviceTypeApi'

const MAX_SIZE = 5 * 1024 * 1024

interface ServiceTypeImageModalProps {
  open: boolean
  onClose: () => void
...
export default function ServiceTypeImageModal({ open, onClose, serviceTypeId, serviceTypeName }: ServiceTypeImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadServiceTypeImageMutation()
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop, accept: { 'image/*': [] }, multiple: false, maxSize: MAX_SIZE,
  })
  ...
  const handleUpload = async () => {
    if (!preview) return
    try {
      await uploadImage({ serviceTypeId, file: preview.file }).unwrap()
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Upload Image" size="sm"
      footer={
        <>
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          Uploading image for <span className="font-medium text-[var(--text-primary)]">{serviceTypeName}</span>
        </p>
        {preview ? (
          ...
          <div {...getRootProps()} className={cn(
            ...
            <p className="text-sm text-[var(--text-secondary)]">{isDragActive ? 'Drop here…' : 'Drag & drop or click to select'}</p>
