...
import { useUploadFqaTypeImageMutation } from '../services/fqaTypeApi'
import { getApiError } from '@/services/apiHelpers'

const MAX_SIZE = 5 * 1024 * 1024

interface FqaTypeImageModalProps {
  open: boolean
  onClose: () => void
  fqaTypeId: number
  fqaTypeName: string
...
export default function FqaTypeImageModal({
  open,
  onClose,
  fqaTypeId,
  fqaTypeName,
}: FqaTypeImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadFqaTypeImageMutation()
  ...
  const handleUpload = async () => {
    if (!preview) return
    try {
      await uploadImage({ fqaTypeId, file: preview.file }).unwrap()
      toast.success(t('fqaType.imageUploadSuccess', 'Image uploaded successfully'))
      handleClose()
    } catch (error: any) {
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          {t('fqaType.uploadFqaTypeImageFor')}{' '}
          <span className="font-medium text-[var(--text-primary)]">{fqaTypeName}</span>
        </p>
