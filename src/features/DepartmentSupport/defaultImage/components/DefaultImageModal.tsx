...
import { useUploadDefaultImageMutation } from '../services/defaultImageApi'
...
export default function DefaultImageModal({ open, onClose, defaultImageId, defaultImageName }: DefaultImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadDefaultImageMutation()
  ...
  const handleUpload = async () => {
    if (!preview) return
    try {
      const data = await uploadImage({ defaultImageId, file: preview.file }).unwrap()
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          Uploading image for <span className="font-medium text-[var(--text-primary)]">{defaultImageName}</span>
        </p>
