...
import { useUploadReelVideoMutation } from '../services/reelApi'
        ...
        {preview ? (
          <div className="relative overflow-hidden rounded-[var(--radius)] border border-[var(--border)]">
            ...
            <button
              ...
              className="absolute top-2 end-2 w-6 h-6 rounded-full bg-[var(--danger)]
          ...
          <div
            ...
            className={cn(
              ...
              'border-2 border-dashed p-8 cursor-pointer text-center transition-all duration-150',
              isDragActive
          ...
          >
            <input {...getInputProps()} />
            <HiFilm className="text-[var(--text-muted)]" size={28} />
            <p className="text-sm text-[var(--text-secondary)]">
              {isDragActive ? 'Drop here…' : 'Drag & drop or click to upload'}
            </p>
            <p className="text-xs text-[var(--text-muted)]">
              Single video · Max {Math.round(MAX_SIZE / 1024 / 1024)} MB
            </p>
            {dropError && (
