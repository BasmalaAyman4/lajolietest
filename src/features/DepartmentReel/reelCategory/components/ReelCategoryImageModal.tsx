...
import { useUploadReelCategoryImageMutation } from '../services/reelCategoryApi'
import { getApiError } from '@/services/apiHelpers'

const MAX_SIZE = 5 * 1024 * 1024

interface ReelCategoryImageModalProps {
  open: boolean
  onClose: () => void
...
export default function ReelCategoryImageModal({
  open,
  onClose,
  reelCategoryId,
  reelCategoryName,
}: ReelCategoryImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadReelCategoryImageMutation()
  ...
  const handleUpload = async () => {
    if (!preview) return
    try {
      await uploadImage({ reelCategoryId, file: preview.file }).unwrap()
  ...
  return (
    <Modal
      open={open}
      onClose={handleClose}
      title="Upload Reel Category Image"
      size="sm"
      footer={
        <>
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          Uploading image for{' '}
          <span className="font-medium text-[var(--text-primary)]">{reelCategoryName}</span>
        </p>
