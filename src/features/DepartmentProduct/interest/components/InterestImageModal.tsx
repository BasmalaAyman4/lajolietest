...
import { useUploadInterestImageMutation } from '../services/interestApi'

const MAX_SIZE = 5 * 1024 * 1024

interface InterestImageModalProps {
  open: boolean
  onClose: () => void
  interestId: number
  interestName: string
...
export default function InterestImageModal({
  open,
  onClose,
  interestId,
  interestName,
}: InterestImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadInterestImageMutation()
  ...
  const handleUpload = async () => {
    if (!preview) return
    try {
      await uploadImage({ interestId, file: preview.file }).unwrap()
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          Uploading image for{' '}
          <span className="font-medium text-[var(--text-primary)]">{interestName}</span>
        </p>
