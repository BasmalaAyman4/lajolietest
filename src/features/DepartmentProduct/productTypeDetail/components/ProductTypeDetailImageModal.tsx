...
interface ProductTypeDetailImageModalProps {
  open: boolean
  onClose: () => void
  ProductTypeDetailId: number
  ProductTypeDetailName: string
}

export default function ProductTypeDetailImageModal({ open, onClose, ProductTypeDetailId, ProductTypeDetailName }: ProductTypeDetailImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadProductTypeDetailImageMutation()
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          Uploading image for <span className="font-medium text-[var(--text-primary)]">{ProductTypeDetailName}</span>
        </p>
        {preview ? (
