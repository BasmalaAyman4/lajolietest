...
import { useUploadPackagingImageMutation } from '../services/packagingApi'

const MAX_SIZE = 5 * 1024 * 1024

interface PackagingImageModalProps {
  open: boolean
  onClose: () => void
  packagingId: number
...
export default function PackagingImageModal({
  open,
  onClose,
  packagingId,
  packagingName,
}: PackagingImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadPackagingImageMutation()
  ...
  const handleUpload = async () => {
    if (!preview) return
    try {
      await uploadImage({ packagingId, file: preview.file }).unwrap()
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          Uploading image for{' '}
          <span className="font-medium text-[var(--text-primary)]">{packagingName}</span>
        </p>
