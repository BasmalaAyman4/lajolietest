...
import { useUploadGoalImageMutation } from '../services/goalApi'

const MAX_SIZE = 5 * 1024 * 1024

interface GoalImageModalProps {
  open: boolean
  onClose: () => void
  goalId: number
  goalName: string
...
export default function GoalImageModal({ open, onClose, goalId, goalName }: GoalImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadGoalImageMutation()
  ...
  const handleUpload = async () => {
    if (!preview) return
    try {
      await uploadImage({ goalId, file: preview.file }).unwrap()
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          Uploading image for <span className="font-medium text-[var(--text-primary)]">{goalName}</span>
        </p>
