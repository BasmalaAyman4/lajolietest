...
interface BundleImageModalProps {
  open: boolean
  onClose: () => void
  bundleId: number
  bundleName: string
...
export default function BundleImageModal({ open, onClose, bundleId, bundleName }: BundleImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadBundleImageMutation()
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Upload Bundle Image" size="sm"
      footer={
        <>
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          Uploading image for <span className="font-medium text-[var(--text-primary)]">{bundleName}</span>
        </p>
        {preview ? (
