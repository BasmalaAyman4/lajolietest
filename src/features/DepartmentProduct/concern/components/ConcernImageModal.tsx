...
import { useUploadConcernImageMutation } from '../services/concernApi'

const MAX_SIZE = 5 * 1024 * 1024

interface ConcernImageModalProps {
  open: boolean
  onClose: () => void
  concernId: number
  concernName: string
...
export default function ConcernImageModal({ open, onClose, concernId, concernName }: ConcernImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadConcernImageMutation()
  ...
  const handleUpload = async () => {
    if (!preview) return
    try {
      await uploadImage({ concernId, file: preview.file }).unwrap()
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          Uploading image for <span className="font-medium text-[var(--text-primary)]">{concernName}</span>
        </p>
        {preview ? (
