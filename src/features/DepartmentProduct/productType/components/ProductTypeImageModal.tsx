...
interface ProductTypeImageModalProps {
  open: boolean
  onClose: () => void
...
export default function ProductTypeImageModal({
  open,
  onClose,
  productTypeId,
  productTypeName,
}: ProductTypeImageModalProps) {
  const { t } = useTranslation()
  const [uploadImage, { isLoading }] = useUploadProductTypeImageMutation()
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--text-muted)]">
          Uploading image for{' '}
          <span className="font-medium text-[var(--text-primary)]">{productTypeName}</span>
        </p>
